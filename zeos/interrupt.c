...
void keyboard_routine(){
  ...
  if(isbreak == 0){ // If it is a make
    ...
    if (toprint == 'x') {
      ...
      if (!b) task_switch((union task_union *)idle_task);
      else task_switch((union task_union *)init_task);
    }

    if (toprint == 'n') {
      b = !b;
      // Verifica si la tecla presionada es la que deseas para el cambio de contexto
      if (!b) task_switch((union task_union *)new);
      else task_switch((union task_union *)init_task);
    }
  }
